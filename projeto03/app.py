...
lojas["data_abertura"] = pd.to_datetime(lojas["data_abertura"])
...
faturamento_mensal_por_loja = df.groupby(["nome_loja", df["data_venda"].dt.month])["valor_total"].sum()
...
lojas_maior_faturamento = df.groupby(["nome_loja"])["valor_total"].sum().sort_values(ascending=False)[:5] # usar reset_index e o  .head(5)
